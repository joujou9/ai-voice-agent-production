...
def prewarm(proc: JobProcess):
    ...
    if response.status_code == 200:
        ...
        logger.info(f"Cartesia API response content: {voices_data}")
        proc.userdata["cartesia_voices"] = voices_data
    else:
...
async def entrypoint(ctx: JobContext):
    ...
    voices = []
    voices_list = cartesia_voices
    
    # Handle different response structures from Cartesia API
    if isinstance(cartesia_voices, dict):
        if "voices" in cartesia_voices:
            voices_list = cartesia_voices["voices"]
        else:
            logger.warning(f"Unexpected Cartesia API response structure: {cartesia_voices}")
            voices_list = []
    
    if isinstance(voices_list, list):
        for voice in voices_list:
            if isinstance(voice, dict) and "id" in voice and "name" in voice:
                voices.append(
                    {
                        "id": voice["id"],  
                        "name": voice["name"],
